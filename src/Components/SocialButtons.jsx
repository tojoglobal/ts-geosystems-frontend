...
const SocialButtons = () => {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 w-full max-w-md mx-auto z-10">
      ...
      {/* Print */}
      <div
        className="flex justify-center  items-center bg-[#343c40] px-1 py-1 h-8"
        style={{ transform: "skewX(-15deg)" }}
      >
        <div
          style={{ transform: "skewX(15deg)" }}
          className="flex items-center text-sm"
        >
          <FaPrint className="text-white text-xs mr-1" />
          <span className="text-white text-[10px]">Print</span>
        </div>
      </div>
    </div>
  );
